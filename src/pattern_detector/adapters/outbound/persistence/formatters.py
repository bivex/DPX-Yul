"""JSON, Markdown, SARIF formatters & persistence repositories for Yul & EVM Assembly."""
...
class MarkdownReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        lines = [
            f"# 🔥 DPX-Yul: Yul & EVM Assembly Architectural Pattern Report",
            f"",
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for det in report.detections:
            ...
            level = "error" if det.pattern_category.value in ("yul_security_hazards", "principle") else "note"
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            "name": "DPX-Yul",
                            "informationUri": "https://github.com/bivex/DPX-Yul",
                            "version": "0.1.0",
                        }
                    },
                    "results": results,
