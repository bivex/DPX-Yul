"""Token-Efficient LLM / AI Prompt Context Formatter for Yul & EVM Assembly."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="yul">',
        ...
        patterns = [d for d in report.detections if d.pattern_category not in (PatternCategory.PRINCIPLE, PatternCategory.YUL_SECURITY_HAZARDS)]
        hazards = [d for d in report.detections if d.pattern_category in (PatternCategory.PRINCIPLE, PatternCategory.YUL_SECURITY_HAZARDS)]

        if patterns:
            lines.append("    <yul_evm_patterns_and_memory_layouts>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <yul_security_hazards_and_memory_corruption>")
            for v in hazards:
