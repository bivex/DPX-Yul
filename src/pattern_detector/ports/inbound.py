"""Inbound driving ports for DPX-Yul."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Yul / EVM Assembly scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".yul", ".sol"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Yul source file."""
